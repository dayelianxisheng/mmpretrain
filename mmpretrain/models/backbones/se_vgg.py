...
def make_se_vgg_layer(in_channels,
                      out_channels,
                      num_blocks,
                      conv_cfg=None,
                      norm_cfg=None,
                      ...
                      ceil_mode=False,
                      se_ratio=16):
    """Build VGG layer with SE blocks."""
    layers = []
    for i in range(num_blocks):
        ...
        layers.append(layer)
        # 在每个卷积层后添加SE模块
        layers.append(SELayer(out_channels, ratio=se_ratio))
...
class SEVGG(BaseBackbone):
    ...
    def __init__(self,
        ...
        super(SEVGG, self).__init__(init_cfg)
