...
class TransformerEncoderLayer(BaseModule):
    ...
    def __init__(self,
        ...
        # ca位置注意力机制
        self.coord_attn = CoordAtt(inp=embed_dims, oup=embed_dims)
    ...
    def forward(self, x, hw_shape=None):
        # Attention residual with CA
        attn_out = self.attn(self.ln1(x))
        if hw_shape is not None:
            # Apply CA to residual: convert (B, N, C) to (B, C, H, W)
            # Separate cls token and patch tokens
            cls_token = attn_out[:, :1, :]  # (B, 1, C)
            patch_tokens = attn_out[:, 1:, :]  # (B, N-1, C)

            B, N, C = patch_tokens.shape
            H, W = hw_shape
            patch_tokens_2d = patch_tokens.reshape(B, H, W, C).permute(0, 3, 1, 2)
            patch_tokens_2d = self.coord_attn(patch_tokens_2d)
            patch_tokens = patch_tokens_2d.permute(0, 2, 3, 1).reshape(B, N, C)

            # Concatenate cls token back
            attn_out = torch.cat([cls_token, patch_tokens], dim=1)
        x = x + attn_out
...
class CAViT(BaseBackbone):
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(CAViT, self).__init__(init_cfg)
