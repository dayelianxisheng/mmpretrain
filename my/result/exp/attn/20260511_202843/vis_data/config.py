...
model = dict(
    ...
    head=dict(
        in_channels=1280,
        loss=dict(
            gamma_neg=4, gamma_pos=0, loss_weight=1.0, type='AsymmetricLoss'),
        num_classes=20,
        topk=1,
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys=dict(backbone=dict(lr_mult=0.1), head=dict(lr_mult=1.0))))
param_scheduler = [
    dict(
        begin=0,
        by_epoch=True,
...
work_dir = './work_dirs/mobilenetv2_attnhead_voc'
