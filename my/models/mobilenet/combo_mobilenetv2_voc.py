# Combo-MobileNetV2 VOC2012 训练配置
# 改进：在 MobileNetV2 的每个 InvertedResidual 后添加 SE + CBAM + CA 三重注意力组合模块
# 训练策略：冻结解冻 + EMA
...
# Combo-MobileNetV2 模型配置
model = dict(
    type='ImageClassifier',
    backbone=dict(
        type='ComboMobileNetV2',
        widen_factor=1.0,
        se_ratio=16,            # SE 模块的通道压缩比
        cbam_ratio=16,          # CBAM 通道注意力的压缩比
        cbam_kernel=3,           # CBAM 空间注意力的卷积核大小
        ca_reduction=32,        # CoordAttention 的通道压缩比
...
# 冻结解冻策略：前5个epoch冻结backbone只训练head和三重注意力模块，解冻后用不同学习率
custom_hooks = [
    dict(
...
# 优化器配置：参数分组，backbone用低学习率，head和注意力模块用正常学习率
optim_wrapper = dict(
    optimizer=dict(
        type='Adam',
        lr=0.00005,
