...
model = dict(
    backbone=dict(
        ...
        init_cfg=dict(
            checkpoint=
            'https://download.openmmlab.com/mmclassification/v0/vit/vit-base-p16_in21k-pre_3rdparty_in1k-384px_20221219-355b19f6.pth',
            map_location='cpu',
            prefix='backbone',
            type='Pretrained'),
        out_indices=-1,
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys=dict({
            ...
            'backbone.layers.*.coord_attn': dict(lr_mult=1.0)
        }),
        norm_decay_mult=0.0))
param_scheduler = [
    dict(
